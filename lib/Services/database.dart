...
class DatabaseServices {
  ...
  final CollectionReference cmpCollection = FirebaseFirestore.instance
      .collection("CMP");
  ...
  Future UpdateCurrentRoom(String CurrentRoom) async {
  ...
  Future<Map> getRoomsHistory() async {
    List history;
    Map rooms = Map<String, String>();
    DocumentReference docref = cmpCollection.doc(uid);
    await docref.get().then((value) {
      history = value['Roomshistory'];
    });
    for (String element in history) {
      rooms[element] = await MenuServices().getRoomName(element);
    }
    return rooms;
  }
